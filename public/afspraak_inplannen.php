...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($date === '' || $time === '' || $minutes <= 0) {
    ...
    if (!$appointmentsModel->isSlotFree($begin->format('Y-m-d H:i:s'), $end->format('Y-m-d H:i:s'), $id)) {
...
<head>
  ...
  <style>
    ...
    h1 { color: #136DEC; }
    .card { background: #f5f5f5; border-radius: 8px; padding: 20px; margin-bottom: 24px; }
    .card h2 { margin: 0 0 12px; font-size: 1rem; color: #444; text-transform: uppercase; letter-spacing: .05em; }
    table { width: 100%; border-collapse: collapse; }
    td { padding: 6px 10px; }
    td:first-child { font-weight: 600; width: 40%; }
    .photo-preview { max-width: 100%; max-height: 220px; border-radius: 4px; margin-top: 8px; }
    label { display: block; font-weight: 600; margin-bottom: 6px; }
    input[type="date"], input[type="time"], input[type="number"] {
      width: 100%; padding: 9px 10px; border: 1px solid #ddd; border-radius: 4px;
      font-size: 14px; margin-bottom: 16px;
    }
    input:focus { outline: none; border-color: #136DEC; box-shadow: 0 0 4px rgba(19,109,236,.3); }
    button[type="submit"] {
      background: #136DEC; color: #fff; border: none; padding: 11px 22px;
      border-radius: 4px; cursor: pointer; font-size: 14px; font-weight: 600;
    }
    button[type="submit"]:hover { background: #0d56c4; }
    .msg-ok  { background: #d4edda; color: #155724; border: 1px solid #c3e6cb; padding: 12px; border-radius: 4px; margin-bottom: 16px; }
    .msg-err { background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; padding: 12px; border-radius: 4px; margin-bottom: 16px; }
    .occupied { margin: 0; padding: 0; list-style: none; }
    .occupied li { background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px;
                   padding: 5px 10px; margin-bottom: 5px; font-size: .9rem; }
    a.back { display: inline-block; margin-top: 20px; color: #136DEC; text-decoration: none; }
...
<body>
  ...
  <div class="card">
    ...
    <table>
      <tr><td>Klant</td><td><?= htmlspecialchars($appointment['customer_name']) ?></td></tr>
      <tr><td>Merk</td><td><?= htmlspecialchars($appointment['brand']) ?></td></tr>
      <tr><td>Model</td><td><?= htmlspecialchars($appointment['model']) ?></td></tr>
      <tr><td>Type reparatie</td><td><?= htmlspecialchars($appointment['type']) ?></td></tr>
      <tr><td>Omschrijving</td><td><?= htmlspecialchars($appointment['description']) ?></td></tr>
      <?php if (!empty($appointment['begintime'])): ?>
        <tr><td>Huidig slot</td><td>
